...
function delete_employment_record (id) {
    if (confirm('Are you you want to delete this record!')) {

        form_data = new Array();
        form_data.push({name: 'form_process' , value: 'delete-working'});
...
function load_employment_history() {
	id = ""
	id = getUrlVars()['id'];	
	...
	$('#datatable').DataTable( {
        ...
        "columnDefs" : [
            ...
            { "ordering" : true, "targets" :4, "orderable" : true, "width" : 150, "order": []},
            { "targets" : 5, "orderable" : false, "searchable" : false, "width" : 50, "className" : "dt-body-right"},
        ],
        "ajax": "library/page/b_l-trainer-profile.php?id=" + id + '&employment',
    });
