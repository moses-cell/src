<?php
	...
	if ($from =='' && $to == '') {
		...
		$condition = "((ts.date_start"  . " between '" . $from ."' and '" . $to . "') or (ts.date_end"  . " between '" . $from ."' and '" . $to . "')) and ";
	} elseif ($to == '' ) {
		$condition = "ts.date_start >= '" . $from . "' and ";
	} elseif ($from == '' ) {
		$condition = "ts.date_end <= '" . $to . "' and ";
	} else {
		$condition = "((ts.date_start"  . " between '" . $from ."' and '" . $to . "') or (ts.date_end"  . " between '" . $from ."' and '" . $to . "')) and ";
	}

	$_SESSION['condition'] = "(ta.status = 'Submit for Approval' or ta.status = 'Approve' or ta.status = 'Submit to HR') and ts.bonding = '1' and ta.participant_type = '1'";

	$condition .= "(ta.status = 'Submit for Approval' or ta.status = 'Approve' or ta.status = 'Submit to HR') and ts.bonding = '1' and ta.participant_type = '1'";
