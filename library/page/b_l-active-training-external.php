<?php
	...
	if ($from == '' && $to == '') {
		$condition = "(ta.status = 'Approve') and ts.date_end >= '" . date('Y-m-d') . "' and participant_type = 2 ";
	
	} elseif ($from == $to) {
		$condition = "(ta.status = 'Approve')  and participant_type = 2 and ((ts.date_start"  . " between '" . $from ."' and '" . $to . "') or (ts.date_end"  . " between '" . $from ."' and '" . $to . "'))";
	} elseif ($to == '' ) {
		$condition = "(ta.status = 'Approve')  and participant_type = 2 and ts.date_end >= '" . $from . "' ";
	} elseif ($from == '' ) {
		$condition = "(ta.status = 'Approve')  and participant_type = 2 and ((ts.date_start"  . " between '" . date('Y-m-d') ."' and '" . $to . "') or (ts.date_end"  . " between '" . date('Y-m-d') ."' and '" . $to . "'))";
	} else {
		$condition = "(ta.status = 'Approve')  and participant_type = 2 and ((ts.date_start"  . " between '" . $from ."' and '" . $to . "') or (ts.date_end"  . " between '" . $from ."' and '" . $to . "'))";
	} 
