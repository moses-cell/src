<?php
	...
	$sql = "select ta.*, tp.provider_name, tl.main_location, tl.sub_location, te.assessment_date, te.id as eval_id, te.eval_status, te.super_name, ";
	...
	$condition = " ta.super_eval > 2";

	if ($from == '' && $to == '') 
		$condition = "ta.super_eval > 2";
	else {
		if ($title != '') {
			$condition = "ta.super_eval > 2 and ts.title = '" . $title . "' and (ts.date_start between '" . $from ."' and '" . $to . "')";	
		} else {
			$condition = "ta.super_eval > 2";
		}
	}
	...
	$columns = array(
		array( 	'db' => 'eval_id',     'dt' => 0, 'tbl_field' => 'eval_id', 
				'formatter' => function( $d, $row ) {
				    ...
				    $edit = '<a href="f-supervisor-assessment.php?eval=' . $code .'">' .			  
  							'<img src="assets/img/edit.png" alt="view supervisor assessment" style="width:16px;height:16px;" title="View Supervisor Assessment"></a>';
	    ...
	    array( 	'db' => 'super_name',  'dt' => 6, 'tbl_field' => 'ta.super_name'),
	  	array( 	'db' => 'assessment_date',  'dt' => 7, 'tbl_field' => 'assessment_date',
			'formatter' => function($d, $row ) {
				$eDate = strtotime($d);
				return date('d/m/Y',$eDate);
			}),
	  	array( 	'db' => 'eval_id',     'dt' => 8, 'tbl_field' => 'te.id', 
				'formatter' => function( $d, $row ) {
				    $glib = new globalLibrary;
				    $code = $glib->encrypt_decrypt('encrypt',$d, 'id');
