...
//	$table .= "inner join eval te on ta.id = te.app_id ";
	...
	$condition = " ta.trainer_eval = 1 and (ta.status = 'Submit for Approval' or ta.status = 'Approve' or ta.status = 'Submit to HR')  and ts.date_end < '" . date('Y-m-d') . "' and ta.total_attend > 0";


					
	$columns = array(
	    array( 	'db' => 'staff_no', 'dt' => 0, 'tbl_field' => 'ta.staff_no' ),
	    ...
	    array( 	'db' => 'trainer_name', 'dt' => 5, 'tbl_field' => 'trainer_name',),
