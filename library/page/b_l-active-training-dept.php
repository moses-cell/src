<?php
	...
	if ($from == '' && $to == '') {
		...
		$main_condition = "(ta.status = 'Submit for Approval' or ta.status = 'Approve' or ta.status = 'Submit to HR')  and participant_type = 1 and ts.date_end >= '" . $from . "' ";
	} elseif ($from == '' ) {
		$main_condition = "(ta.status = 'Submit for Approval' or ta.status = 'Approve' or ta.status = 'Submit to HR')  and participant_type = 1 and ((ts.date_start"  . " between '" . date('Y-m-d') ."' and '" . $to . "') or (ts.date_end"  . " between '" . date('Y-m-d') ."' and '" . $to . "'))";
	} else {
		$main_condition = "(ta.status = 'Submit for Approval' or ta.status = 'Approve' or ta.status = 'Submit to HR')  and participant_type = 1 and ((ts.date_start"  . " between '" . $from ."' and '" . $to . "') or (ts.date_end"  . " between '" . $from ."' and '" . $to . "'))";
	}
