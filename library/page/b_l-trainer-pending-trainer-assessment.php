...
//	$table .= "inner join eval te on ta.id = te.app_id ";
	...
	$key = " (ta.staff_no =:appr_no and (ta.status = 'Submit for Approval' or ta.status = 'Approve') and ta.eval = 1 and ta.total_attend > 0) ";

	$condition = " (ta.trainer_eval = 1 and ta.status != 'Cancel' and ts.trainer_id = '" . $_SESSION['trainer_id'] . "' and ta.total_attend > 0) ";    


					
	$columns = array(
		array( 	'db' => 'id', 'dt' => 0, 'tbl_field' => 'ta.id',
	    ...
	    array( 	'db' => 'id', 'dt' => 6, 'tbl_field' => 'ta.id',
			'formatter' => function($d, $row ) {
				$glib = new globalLibrary();
				$code = $glib->encrypt_decrypt('encrypt',$d, 'id');
				$edit = '<a href="f-trainer-assessment.php?id=' . $code .'">' .			  
