<?php
	...
	if ($from == '' && $to == '') {
		...
		$condition = "(ta.status = 'Submit for Approval' or ta.status = 'Approve' or ta.status = 'Submit to HR')  and participant_type = 1 and ((ts.date_start"  . " between '" . $from ."' and '" . $to . "') or (ts.date_end"  . " between '" . $from ."' and '" . $to . "'))";
	} elseif ($to == '' ) {
		$condition = "(ta.status = 'Submit for Approval' or ta.status = 'Approve' or ta.status = 'Submit to HR')  and participant_type = 1 and ts.date_end >= '" . $from . "' ";
	} elseif ($from == '' ) {
		$condition = "(ta.status = 'Submit for Approval' or ta.status = 'Approve' or ta.status = 'Submit to HR')  and participant_type = 1 and ((ts.date_start"  . " between '" . date('Y-m-d') ."' and '" . $to . "') or (ts.date_end"  . " between '" . date('Y-m-d') ."' and '" . $to . "'))";
	} else {
		$condition = "(ta.status = 'Submit for Approval' or ta.status = 'Approve' or ta.status = 'Submit to HR')  and participant_type = 1 and ((ts.date_start"  . " between '" . $from ."' and '" . $to . "') or (ts.date_end"  . " between '" . $from ."' and '" . $to . "'))";
	} 
	...
	if (isset($_GET['staff_name'])) {
		if ($_GET['staff_name'] != '') {
			$condition .= " and fullname like '%" . $_GET['staff_name'] . "%'";
		}
	}
