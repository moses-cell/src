<?php
	...
	$condition = "(ta.status = 'Cancel')  and participant_type = 1";

	if ($from == '' && $to == '') {
		$main_condition = "(ta.status = 'Cancel')  and participant_type = 1";
	
	} elseif ($from == $to) {
		$main_condition = "(ta.status = 'Cancel')  and participant_type = 1 and ((ts.date_start"  . " between '" . $from ."' and '" . $to . "') or (ts.date_end"  . " between '" . $from ."' and '" . $to . "')) ";
	} elseif ($to == '' ) {
		$main_condition = "(ta.status = 'Cancel')  and participant_type = 1 and ts.date_start >= '" . $from . "'  ";
	} elseif ($from == '' ) {
		$main_condition = "(ta.status = 'Cancel')  and participant_type = 1 and ts.date_end <= '" . $to . "' ";
	} else {
		$main_condition = "(ta.status = 'Cancel')  and participant_type = 1 and ((ts.date_start"  . " between '" . $from ."' and '" . $to . "') or (ts.date_end"  . " between '" . $from ."' and '" . $to . "'))";
	}
	...
	$columns = array(
	    ...
	    array( 	'db' => 'cancel_status',  'dt' => 8, 'tbl_field' => 'ta.cancel_status' ),
