<?php
	...
	echo '<p>sch_training-second-reminder start'. '</p>';

	$c_setting = new c_adm_setting();
	$setting_data = $c_setting->_get_record_by_key('training_second_reminder');
	if (count($setting_data) <= 0 ) {
		echo '<p>No setting found for training_second_reminder ' . '</p>' ;
		echo '<p>sch_training-second-reminder stop'. '</p>';
		return;
	}
